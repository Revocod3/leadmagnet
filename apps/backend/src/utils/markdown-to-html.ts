...
export function convertDiagnosisToHTML(markdown: string): string {
  ...
  html = html.replace(/^### (.+)$/gm, '<h3 class="text-xl font-bold text-brand-green-700 dark:text-brand-green-400 mb-4 mt-6">$1</h3>');

  // Convertir encabezados H4 (#### Título)
  html = html.replace(/^#### (.+)$/gm, '<h4 class="text-lg font-semibold text-neutral-800 dark:text-neutral-200 mb-3 mt-4">$1</h4>');
  ...
  html = html.replace(/^✅ (.+)$/gm, '<div class="flex items-start gap-2 mb-2"><span class="text-brand-green-500 text-lg flex-shrink-0">✅</span><span class="text-neutral-700 dark:text-neutral-300">$1</span></div>');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^- (.+)$/)) {
      ...
      processedLines.push(`<li class="text-neutral-700 dark:text-neutral-300">${content}</li>`);
    } else {
      if (inList) {
  ...
  html = html.split('\n').map(line => {
    ...
    // Convertir a párrafo
    return `<p class="mb-3 text-neutral-700 dark:text-neutral-300 leading-relaxed">${trimmed}</p>`;
  }).join('\n');

  // Agregar contenedor especial para el título principal
  html = html.replace(
    /<h3 class="text-xl font-bold text-brand-green-700 dark:text-brand-green-400 mb-4 mt-6">🔬 DIAGNÓSTICO INTEGRAL<\/h3>/,
    '<div class="bg-gradient-to-r from-brand-green-50 to-green-100 dark:from-neutral-800 dark:to-neutral-700 rounded-xl p-6 mb-6 shadow-md"><h3 class="text-2xl font-bold text-brand-green-700 dark:text-brand-green-400 mb-0">🔬 DIAGNÓSTICO INTEGRAL</h3></div>'
  );
